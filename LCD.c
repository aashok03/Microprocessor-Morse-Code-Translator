...
void LCD_Configure(void){
	...
	// Wait until the LCD is enabled by checking the ENS bit of LCD_SR
	LCD->SR &= ~(LCD_SR_ENS);
	LCD->SR |= LCD_SR_ENS;
	
	// Wait until the LCD booster is ready by checking the RDY bit of LCD_SR
	LCD->SR &= ~(LCD_SR_RDY);
	LCD->SR |= LCD_SR_RDY;	
...
void LCD_DisplayString(uint8_t* ptr){
	...
	while (ptr && count < 6)
	{
		LCD_WriteChar(&ptr[count], 0, 0, count);
		count ++;
	}		
...
void LCD_Display_Name(void){
	...
	LCD_DisplayString ((uint8_t *)"AASHOK");
